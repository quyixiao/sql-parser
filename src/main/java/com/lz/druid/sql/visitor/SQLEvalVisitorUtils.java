...
import com.lz.druid.sql.dialect.db2.visitor.DB2EvalVisitor;
import com.lz.druid.sql.dialect.mysql.visitor.MySqlEvalVisitorImpl;
...
public class SQLEvalVisitorUtils {
    ...
    public static boolean visit(SQLEvalVisitor visitor, SQLMethodInvokeExpr x) {
        ...
        if ("mod".equals(methodName)) {
            ...
            if (result >= Integer.MIN_VALUE && result <= Integer.MAX_VALUE) {
                ...
                x.putAttribute(SQLEvalVisitor.EVAL_VALUE, intResult);
            } else {
            ...
            x.putAttribute(SQLEvalVisitor.EVAL_VALUE, Math.PI);
        } else if ("rand".equals(methodName)) {
            x.putAttribute(SQLEvalVisitor.EVAL_VALUE, Math.random());
            ...
            if (firstResult instanceof Number) {
                ...
                x.putAttribute(SQLEvalVisitor.EVAL_VALUE, Character.toString(ch));
            }
        } else if ("current_user".equals(methodName)) {
            x.putAttribute(SQLEvalVisitor.EVAL_VALUE, "CURRENT_USER");
        }
        return false;
    }
    ...
    public static boolean visit(SQLEvalVisitor visitor, SQLHexExpr x) {
        ...
        if (bytes == null) {
            x.putAttribute(SQLEvalVisitor.EVAL_VALUE, SQLEvalVisitor.EVAL_ERROR);
        } else {
    ...
    public static boolean visit(SQLEvalVisitor visitor, SQLBetweenExpr x) {
        ...
        if (lt(value, begin)) {
            x.getAttributes().put(SQLEvalVisitor.EVAL_VALUE, x.isNot() ? true : false);
            return false;
        }
        ...
        x.getAttributes().put(SQLEvalVisitor.EVAL_VALUE, x.isNot() ? false : true);
        return false;
    }

    public static boolean visit(SQLEvalVisitor visitor, SQLNullExpr x) {
        x.getAttributes().put(SQLEvalVisitor.EVAL_VALUE, SQLEvalVisitor.EVAL_VALUE_NULL);
        return false;
    }
    ...
    public static boolean visit(SQLEvalVisitor visitor, SQLUnaryExpr x) {
        ...
        if (val == null) {
            x.putAttribute(SQLEvalVisitor.EVAL_VALUE, SQLEvalVisitor.EVAL_VALUE_NULL);
            return false;
        }
        ...
        switch (x.getOperator()) {
            ...
            case Not: {
                ...
                if (booleanVal != null) {
                    x.putAttribute(SQLEvalVisitor.EVAL_VALUE, !booleanVal);
                }
                break;
            }
            case Plus:
                ...
                break;
            case Negative:
                x.putAttribute(SQLEvalVisitor.EVAL_VALUE, multi(val, -1));
                break;
            case Compl:
                x.putAttribute(SQLEvalVisitor.EVAL_VALUE, ~castToInteger(val));
                break;
            default:
                break;
        }
    ...
    public static boolean visit(SQLEvalVisitor visitor, SQLBinaryOpExpr x) {
        ...
        boolean leftHasValue = left.getAttributes().containsKey(SQLEvalVisitor.EVAL_VALUE);
        boolean rightHasValue = right.getAttributes().containsKey(SQLEvalVisitor.EVAL_VALUE);

        if ((!leftHasValue) && !rightHasValue) {
            SQLExpr leftEvalExpr = (SQLExpr) left.getAttribute(SQLEvalVisitor.EVAL_EXPR);
            SQLExpr rightEvalExpr = (SQLExpr) right.getAttribute(SQLEvalVisitor.EVAL_EXPR);
        ...
        switch (x.getOperator()) {
            ...
            case IsNot:
                if (leftValue == SQLEvalVisitor.EVAL_VALUE_NULL) {
                    x.putAttribute(SQLEvalVisitor.EVAL_VALUE, false);
                } else if (leftValue != null) {
                    x.putAttribute(SQLEvalVisitor.EVAL_VALUE, true);
                }
                break;
            case RegExp:
            case RLike: {
            ...
            case BooleanAnd: {
                ...
                x.putAttribute(SQLEvalVisitor.EVAL_VALUE, first && second);
                break;
            }
            case BooleanOr: {
                boolean first = eq(leftValue, true);
    ...
    public static boolean visit(SQLEvalVisitor visitor, SQLVariantRefExpr x) {
        ...
        if (varIndex != -1 && visitor.getParameters().size() > varIndex) {
            boolean containsValue = attributes.containsKey(SQLEvalVisitor.EVAL_VALUE);
            if (!containsValue) {
